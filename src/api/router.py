...
from src.Agents.ChatBot.Db.__init__ import get_db
from src.Agents.ChatBot.Db.models import ConversationMode,MessageRole,ProcessingStatus,User,Conversation,Message,Document,ConvDocumentLink,DocumentChunk



import uuid
from src.Agents.ChatBot.rag_service import delete_documents_for_conversation
from src.Agents.ChatBot import rag_service
from src.Agents.ChatBot import service as conversation_service
from src.Agents.ChatBot import document_processor 
from src.Agents.ChatBot.Db.Schema import (
    ConversationCreate,
...
@router.post("/documents/url", status_code=status.HTTP_201_CREATED, tags=["Documents"])
async def upload_url(
    ...
    """
    Receives a URL, fetches content, processes it,
    and returns the resulting document ID.
    """
    try:
        document_id = document_processor.process_url(db, user_id, None, payload.url)
        ...
        return {
            "message": "URL processed successfully. ID saved for linking.",
            "document_id": document_id,
            "url": payload.url
        }
    except HTTPException as e:
        ...
        logger.error(f"Uncaught exception during URL processing: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"An internal server error occurred during processing: {str(e)}"
        )